...
const Home = () => {
  ...
  return (
    <div className="overflow-hidden bg-custom-primary min-h-screen">
      <section ref={heroRef} className="relative h-[95vh] flex items-center bg-custom-primary px-4 sm:px-6 lg:px-8 transition-colors duration-500">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center w-full">
          <div className="space-y-8 z-10">
            <span className={`hero-badge inline-block px-4 py-1.5 text-sm font-semibold rounded-full ${theme === 'gold' ? 'bg-[#d4af37]/20 text-[#d4af37]' : 'bg-indigo-100 text-indigo-700'
              }`}
            >
              New Collection 2024
            </span>
          ...
          <div className="hero-img relative hidden lg:block perspective-1000">
            ...
            <img
              ...
              alt="Hero Product"
              className={`relative rounded-[40px] shadow-2xl transform rotate-3 hover:rotate-0 transition-transform duration-1000 border-4 ${
                theme === 'gold' ? 'border-[#d4af37]/30 shadow-[#d4af37]/20' : 
                theme === 'light' ? 'border-custom shadow-indigo-500/10' : 
      ...
      <section className="py-20 bg-custom-primary border-y border-custom-secondary relative z-20">
        <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {[
            ...
            <div key={i} className={`flex items-center space-x-5 p-6 rounded-[32px] transition-all cursor-default border ${
                theme === 'light' ? 'bg-white border-custom hover:shadow-xl' : 'bg-custom-secondary/30 border-transparent hover:border-custom'
              }`}>
              ...
              latestProducts.map((product: any) => (
                <div key={product.id} className="featured-card group bg-custom-primary rounded-[32px] overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 border border-custom relative">
                  ...
                  <button
                    ...
                    className={`absolute top-5 right-5 p-3 rounded-full backdrop-blur-md transition-all z-20 ${isInWishlist(product.id) ? 'bg-red-500 text-white' : 'bg-white/20 text-white hover:bg-white/40'
                      }`}
                  >
                  ...
                  <div className="p-8 space-y-4">
                    ...
                    <div className="flex justify-between items-center pt-2">
                      ...
                      <button
                        ...
                        className={`px-6 py-3 rounded-2xl font-bold transition-all transform active:scale-90 ${theme === 'gold' ? 'bg-[#d4af37] text-black hover:bg-[#b8860b]' : 'bg-gray-900 text-white hover:bg-black shadow-lg shadow-black/10'
                          }`}>
                        Add
                      </button>
                    </div>
      ...
      <section className="py-24 px-4">
        <div className={`max-w-7xl mx-auto rounded-[60px] p-12 md:p-24 text-center space-y-8 relative overflow-hidden transition-all duration-500 ${theme === 'gold'
            ? 'bg-[#d4af37] text-black shadow-[#d4af37]/40 shadow-2xl'
            : theme === 'dark'
              ? 'bg-gray-900 text-white shadow-2xl shadow-black/30'
              : 'bg-custom-secondary border-4 border-custom text-custom-primary shadow-xl'
          }`}>
            ...
            <form className="max-w-md mx-auto flex flex-col sm:flex-row gap-4 pt-6" onSubmit={(e) => e.preventDefault()}>
              <input
                ...
                className={`flex-1 px-8 py-5 rounded-3xl font-bold bg-white/10 backdrop-blur-md border border-white/20 focus:outline-hidden focus:ring-2 focus:ring-white/50 placeholder:text-white/40 ${theme === 'gold' ? 'bg-black/10 text-black border-black/20 placeholder:text-black/40' :
                    theme === 'light' ? 'bg-custom-primary text-custom-primary border-custom placeholder:text-custom-secondary/50' :
                      ''}`}
              />
              <button className={`px-10 py-5 rounded-3xl font-black transition-all transform hover:scale-105 active:scale-95 shadow-lg ${theme === 'gold' ? 'bg-black text-white hover:bg-gray-800' :
                  theme === 'light' ? 'bg-accent text-white hover:opacity-90' :
          ...
          <div className={`absolute top-0 right-0 w-[500px] h-[500px] opacity-10 blur-[120px] -translate-y-1/2 translate-x-1/2 rounded-full ${theme === 'light' ? 'bg-accent' : 'bg-white'}`}></div>
          <div className={`absolute bottom-0 left-0 w-[300px] h-[300px] opacity-10 blur-[80px] translate-y-1/2 -translate-x-1/2 rounded-full ${theme === 'light' ? 'bg-accent' : 'bg-black'}`}></div>
        </div>
      </section>
    </div>
  );
